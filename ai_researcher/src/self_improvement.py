...
def self_improve(experiment_plan, paper_bank, openai_client, model, seed):
    ...
    prompt = "You are a researcher specialized in Natural Language Processing. You have a research project proposal from a collaborator asking your help to improve it.\n"
    prompt += "The project proposal is:\n" + experiment_plan.strip() + ".\n"
    prompt += "There are a set of related papers provided for you:\n" + format_papers_for_printing(paper_bank[:5]) + "\n"
    prompt += "Now make some minor edits to the original proposal to better highlight the novelty. If the proposed idea is sufficiently different from all provided papers, you don't have to edit anything. If not, you should either highlight how the proposed idea is novel or make some necessary modifications to make the proposed idea more novel.\n"
    prompt += "You should preserve the original experiment plan as much as possible. But if you added additional ideas to improve novelty, then you should incoporate them in the experiment plan accordingly. You should not delete any details from the original experiment plan, and are only allowed to add additional details to match the updated problem statement when necessary.\n"
