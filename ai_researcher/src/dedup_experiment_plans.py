...
if __name__ == "__main__": 
    ...
    parser.add_argument('--similarity_threshold', type=float, default=0.85, help='NN Similarity Threshold')
    ...
    for i in range(len(all_filenames)):
        if i not in filter_idx:
            ...
            for j in range(i+1, len(all_filenames)):
                if j not in filter_idx and similarity_matrix[i][j] > args.similarity_threshold or all_filenames[j] == all_filenames[i]:
                    filter_idx.append(j)
    ...
    for filename in final_ideas:
        shutil.copy(os.path.join(args.cache_dir, args.cache_name, filename), os.path.join(args.dedup_cache_dir, args.cache_name, filename))
