...
def style_transfer(model_idea, human_idea, openai_client, model, seed):
    ...
    prompt += "The main sections should be indexed clearly without indentation at the beginning. The title section does not need indexing, other sections including problem statement, motivation, proposed method, step-by-step experiment plan, test case examples, and fallback plan should be indexed 1 to 6. Each section can then have sub-bullets for sub-sections if applicable.\n"
    ...
    prompt += "For in-line citations, if the citation mentioned the author name (like \"(Si et al., 2024)\"), you can leave it as is; but if the citation is just a number (like \"[1]\" or \"[3,4,5]\"), you should just remove it and do some necessary rephrasing to make the sentence still sound coherent without the reference.\n"
